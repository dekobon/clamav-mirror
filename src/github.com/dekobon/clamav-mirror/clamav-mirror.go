...
func parseCliFlags() (bool, string) {
	...
	dataFilePart := getopt.StringLong("data-file-path", 'd', "/var/clamav/data", "Path to ClamAV data files");
	getopt.Parse();

	if (!exists(*dataFilePart)) {
		msg := fmt.Sprintf("Data file path doesn't exist or isn't accessible: %v", *dataFilePart);
		logFatal.Fatal(msg);
	}
	...
	if (err != nil) {
		msg := fmt.Sprintf("Unable to parse absolute path of data file path: %v", *dataFilePart);
		logFatal.Fatal(msg);
	}
